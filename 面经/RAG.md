...
![697](assets/RAG/file-20260505135658892.png)

6.RAG检索优化
![](assets/RAG/file-20260505145603957.png)
...
8.Naive RAG -> Advanced RAG -> Modular RAG

**Self RAG**
Self-RAG 引入了“反思令牌”（Reflection Tokens）。它训练模型在生成文本的过程中，自动插入一些特殊的标签，用来评价检索到的内容以及自己生成的回答。
这些反思令牌主要分为四类：
- IsRel (Is Relevant): 检索到的文档对解决问题真的有用吗？
- IsSup (Is Supported): 生成的回答内容是否真的得到了文档的支持？（防幻觉）
- IsUse (Is Useful): 最终生成的回答对用户是否有用？
- Retrieve: 是否需要进行检索？（判断问题的复杂度）
反思令牌并不是简单的文本字符串，它们在技术上被处理为特殊的词表词元（Special Tokens）。
- 添加到词表： 开发者会将 [Retrieve]、[IsRel]、[IsSup] 等标签作为新的特殊 Token 添加到大模型的词表（Tokenizer）中。
- 作为生成目标： 与普通单词一样，模型在预测下一个词时，不仅会预测“苹果”、“科学”等词，也会在特定的上下文位置预测出 [Relevant] 或 [Supported]。
----
**Corrective RAG**
**Graph RAG**
**Agentic RAG**
![](assets/RAG/file-20260505153708471.png)